...
public class PlayersController implements Serializable {
    ...
    public PaginationHelper getPagination() {
        if (pagination == null) {
            pagination = new PaginationHelper(ONE_PAGE_DATA_UNIT) {
                ...
                public DataModel createPageDataModel() {
                    return new ListDataModel(getFacade().findRange(new int[]{getPageFirstItem(), getPageFirstItem() + getPageSize() - 1})); //1based補正
                }
            };
        }
        return pagination;
